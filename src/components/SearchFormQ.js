...
const SearchFormQ = props => {
	...
	useEffect(() => {
		if (props.ticketsQ != null) {
			const results = props.ticketsQ.filter(
				ticket =>
	...
	if (props.ticketsQ === null) {
		...
		return (
			<Center>
				<Div1>
					<div id="expandListTextQ" style={{ color: '#86929d', fontSize: '0.75rem', fontStyle: 'italic' }}>
						click header to show ticket queue
					</div>
					...
					<Form id="searchFormQ" style={{ display: 'none', height: '0%' }}>
						...
						{searchResultsQ.map(ticket => (
							<TicketQ key={ticket.ticketsid} ticket={ticket} />
						))}
					</Form>
				</Div1>
			</Center>
