...
const SearchForm = props => {
	...
	useEffect(() => {
		if (props.tickets !== null) {
			const results = props.tickets.filter(
				ticket =>
	...
	console.log('profile id = ' + props.profile.usersid);
	...
	if (props.tickets === null) {
		...
		return (
			<Center>
				<Div1>
					<div id="expandListText" style={{ color: '#86929d', fontSize: '0.75rem', fontStyle: 'italic' }}>
						click header to hide your tickets
					</div>
					...
					<Form id="searchForm" style={{ display: 'none', height: '0%' }}>
						...
						{searchResults.map(ticket => (
							<Ticket key={ticket.ticketsid} ticket={ticket} />
						))}
					</Form>
				</Div1>
			</Center>
